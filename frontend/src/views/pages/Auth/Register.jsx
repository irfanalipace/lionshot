...
export default function Register() {
   ...
   const [loading, setLoading]=useState(false)
	...
	const handleAccountSubmit = async (values, { setSubmitting }) => {
		try {
		  ...
		  function cb(params) {
			if (params === 'catch') {
			  ...
			  console.log('Registration successful:', params);  // Log the response
	  ...
	  useEffect(() => {
		if (apiError) {
		  const { email, first_name, last_name, password, password_confirmation } = apiError;
		  ...
		  if (password && password.length > 0 && !password[0].includes('confirmation')) {
				...
				<Grid container>
					...
					<Grid container display='flex' justifyContent='center'>
						...
						<Grid item sx={{display:'flex', justifyContent:'center'}}>
							<img src={LoginLogo} alt=''  style={{height:'36px', marginBottom: '28px'}}/>
							</Grid>
