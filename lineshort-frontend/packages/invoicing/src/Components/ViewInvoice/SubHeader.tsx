...
import { generateEncryptedID } from '@/utils/helper';

const BtnStyles = {
	margin: '0 .2rem',
};

export default function SubHeader({ status, handleNewInvoice, id }) {
	const navigate = useNavigate();
	const [emailMenuAnchor, setEmailMenuAnchor] = useState(null);
	const [pdfLoading, setPdfLoading] = useState(false);

	const openEmailMenu = (event: any) => {
	...
	const handleSendEstimateMain = (id: any) => {
