...
import { createTax } from '@/apis/others/tax';

interface NewTaxModalProps {
	open: boolean;
	onClose: () => void;
	setIsNewTaxAdded: (isNew: boolean) => void;
}

const NewTaxModal: React.FC<NewTaxModalProps> = ({
	open,
	onClose,
	setIsNewTaxAdded,
}) => {
	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			console.log('NewTaxModal Submitted ', values);
			setLoading(true);
			try {
				const resp: any = await createTax(values);
				if (resp?.success === true) {
					setIsNewTaxAdded(true);
				}
				notyf.success(resp?.message);
			} catch (err: any) {
				console.error('NewTaxModal error', err);
